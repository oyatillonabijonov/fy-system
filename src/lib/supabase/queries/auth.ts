...
export async function getCurrentMemberClient(): Promise<MemberClient | null> {
  const { data: { user } } = await supabase.auth.getUser()
  if (!user) return null

  // Check clients table directly — works for both email/password and Google OAuth members.
  // (user_type metadata check removed: Google OAuth doesn't set it.)
  // This is only reached when getCurrentProfile() returned null, so staff profiles
  // won't appear here — they always have a profiles row.
  const { data, error } = await supabase
    .from("clients")
    .select("id, full_name, email, phone, company, image, auth_user_id, community_approved, created_at")
  ...
  return {
    ...
    community_approved: data.community_approved ?? false,
    created_at: data.created_at,
  }
}

export async function selfRegisterMember(input: {
  email: string
  password: string
  full_name: string
  phone: string
  company: string
  image_url?: string
}): Promise<{ user_id: string; client_id: string }> {
  const supabaseUrl = import.meta.env.VITE_SUPABASE_URL as string
  const supabaseAnonKey = import.meta.env.VITE_SUPABASE_ANON_KEY as string
  const functionUrl = `${supabaseUrl}/functions/v1/member-self-register`
  ...
  try {
    ...
    throw new Error(err instanceof Error ? err.message : "Ulanishda xatolik")
  }

  let payload: { ok?: boolean; user_id?: string; client_id?: string; error?: string } = {}
  try { payload = await response.json() } catch { /* non-JSON */ }

  if (!response.ok) throw new Error(payload.error ?? `Xatolik (HTTP ${response.status})`)
  if (!payload.user_id) throw new Error("Server user_id qaytarmadi")

  return { user_id: payload.user_id, client_id: payload.client_id! }
}
